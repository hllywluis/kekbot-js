            ...
            if (err) {
                ...
                if (def) {
                    const dictionaryEmbed = new d.MessageEmbed()
                        ...
                        .addField('Definition by ' + def[0].author.replace(/\[/g, '').replace(/]/g, '') + ':', def[0].definition >= 1024 ? def[0].definition.replace(/\[/g, '').replace(/]/g, '').substr(0, 1020) + '...' : def[0].definition.replace(/\[/g, '').replace(/]/g, ''))
                        .addField('Example:', def[0].example >= 1024 ? def[0].example.replace(/\[/g, '').replace(/]/g, '').substr(0, 1020) + '...' : def[0].example.replace(/\[/g, '').replace(/]/g, ''))
